...
来源:[http://gank.io/](http://gank.io/post/560e15be2dca930e00da1083)

作者：[扔物线](https://github.com/rengwuxian)

<a href="http://www.jobtong.com/jobs/0.0.30.0.0.0.0">
![](http://ww1.sinaimg.cn/large/610dc034gw1ewwdtf8l8ej20qo046myh.jpg)
</a>
...
[liuhuofenglin]:http://weibo.com/wowwing
[daimajia]:http://weibo.com/daimajia
[baoyongzhang]:http://weibo.com/u/3224930551
[drakeet]:https://github.com/drakeet
[malin]:https://github.com/androidmalin
[youshifangzong]:http://weibo.com/kevingracie
[chengxuyifeiyuan]:http://weibo.com/u/2559577095
[datougui]:http://weibo.com/brucefromsdu
[XZoomEye]:http://weibo.com/u/2662418685
[xideyu]:http://weibo.com/shadev
[TCahead]:http://weibo.com/tcahead
[Tiiime]:http://weibo.com/u/1963663403
[Ailurus]:http://weibo.com/208087666
[zhaixuezhang]:http://weibo.com/seniorszhai
[yaonie]:http://naotou.github.io/
[dachenge]:http://weibo.com/u/3214362483
[NicodeLee]:https://github.com/rizhilee
[jobtong]:http://jobtong.com/

另外，感谢 RxJava 核心成员[流火枫林][liuhuofenglin]的技术支持和内测读者[代码家][daimajia]、[鲍永章][baoyongzhang]、[drakeet][drakeet]、[马琳][malin]、[有时放纵][youshifangzong]、[程序亦非猿][chengxuyifeiyuan]、[大头鬼][datougui]、[XZoomEye][XZoomEye]、[席德雨][xideyu]、[TCahead][TCahead]、[Tiiime][Tiiime]、[Ailurus][Ailurus]、[宅学长][zhaixuezhang]、[妖孽][yaonie]、[大大大大大臣哥][dachenge]、[NicodeLee][NicodeLee]的帮助，以及[周伯通招聘][jobtong]的赞助。
...
![](RxJavaForAndroid/rxjava_give_an_example.png)
...
> 
> * Lambda 是把双刃剑，它让你的代码简洁的同时，降低了代码的可读性，因此同时学习 RxJava 和 Retrolambda 可能会让你忽略 RxJava 的一些技术细节；
...
![](RxJavaForAndroid/rxjava_button_onclick.png)
...
![](RxJavaForAndroid/rxjava_observable_mode.png)
...
![](RxJavaForAndroid/rxjava_observable_onnext_oncompleted_onerror.png)
...
![](RxJavaForAndroid/rxjava_observable_subscribe.png)

或者可以看动图：

![](RxJavaForAndroid/rxjava_observable_subscribe_gif.gif)
...
// 自动创建 Subscriber ，并使用 onNextAction 和 
// onErrorAction 来定义 onNext() 和 onError()
...
![](RxJavaForAndroid/rxjava_this_is_unless.png)
...
![](RxJavaForAndroid/rxjava_lift_map.png)
...
![](RxJavaForAndroid/rxjava_lift_flatmap.png)
...
![](RxJavaForAndroid/rxjava_lift_principle.png)

或者可以看动图：

![](RxJavaForAndroid/rxjava_lift_principle_gif.gif)

两次和多次的`lift()`同理，如下图：

![](RxJavaForAndroid/rxjava_lift_multiple.png)
...
讲述`lift()`的原理只是为了让你更好地了解`RxJava`，从而可以更好地使用它。然而不管你是否理解了`lift()`的原理，RxJava都不建议开发者自定义`Operator`来直接使用`lift()`，而是建议尽量使用已有的`lift()`包装方法（如`map()` `flatMap()`等）进行组合来实现需求，因为直接使用`lift()`非常容易发生一些难以发现的错误。
...
![](RxJavaForAndroid/rxjava_subscribeon_principle.png)

`observeOn()`原理图：

![](RxJavaForAndroid/rxjava_observeon_principle.png)
...
![](RxJavaForAndroid/rxjava_observeon_subscribeon_multiple.png)
...
[RxBinding][RxBinding] 是 `Jake Wharton` 的一个开源库，它提供了一套在 `Android` 平台上的基于`RxJava`的`Binding API`。所谓`Binding`，就是类似设置`OnClickListener`、设置`TextWatcher`这样的注册绑定对象的 API。
...
如果想对 RxBinding 有更多了解，可以去它的 [GitHub 项目][RxBinding] 下面看看。
...
[RxBus]:http://nerds.weddingpartyapp.com/tech/2014/12/24/implementing-an-event-bus-with-rxjava-rxbus/

`RxBus`名字看起来像一个库，但它并不是一个库，而是一种模式，它的思想是使用`RxJava`来实现了`EventBus`，而让你不再需要使用`Otto`或者`GreenRobot`的`EventBus`。至于什么是`RxBus`，可以看[这篇文章][RxBus]。顺便说一句，`Flipboard`已经用`RxBus`替换掉了`Otto`，目前为止没有不良反应。
...
再次感谢对这篇文章的产出提供支持的各位： 
技术支持：[流火枫林][liuhuofenglin]

内测读者：[代码家][daimajia]、[鲍永章][baoyongzhang]、[drakeet][drakeet]、[马琳][malin]、[有时放纵][youshifangzong]、[程序亦非猿][chengxuyifeiyuan]、[大头鬼][datougui]、[XZoomEye][XZoomEye]、[席德雨][xideyu]、[TCahead][TCahead]、[Tiiime][Tiiime]、[Ailurus][Ailurus]、[宅学长][zhaixuezhang]、[妖孽][yaonie]、[大大大大大臣哥][dachenge]、[NicodeLee][NicodeLee]

赞助方：[周伯通招聘][jobtong]是他们让我的文章能够以不那么丑陋的样子出现在大家面前。

### 关于作者

朱凯（扔物线），[Flipboard](https://www.flipboard.cn/)北京 Android 工程师。
...
![](RxJavaForAndroid/rxjava_qrcode.png)
...
版权声明：自由转载 - 不可商用 - 不可衍生 - 保持署名 (请遵守：[CreativeCommons 3.0协议](https://creativecommons.org/licenses/by-nc-nd/3.0/deed.zh))
